...
function initUniversalGlobalStore() {
  const projectSkillDir = path.resolve(rootDir, ".agents", "skills", "memory-store");
  console.log(`\n  📁 Project-level install: ${projectSkillDir}`);
  const ok = installTo(projectSkillDir);

  // Also init the project memory store
  const memoryStoreDir = path.resolve(rootDir, ".agents", "memory-store");
  const memFile = path.join(memoryStoreDir, "memories.json");
  if (!fs.existsSync(memFile)) {
    fs.mkdirSync(memoryStoreDir, { recursive: true });
    fs.writeFileSync(memFile, "[]", "utf-8");
    fs.writeFileSync(path.join(memoryStoreDir, "memories.index.json"), "{}", "utf-8");
    console.log(`    ✅ Initialized empty memory store: ${memoryStoreDir}`);
  } else {
    console.log(`    ⏭️  Memory store already exists: ${memoryStoreDir}`);
  }
